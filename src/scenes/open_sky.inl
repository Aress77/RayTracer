...
Scene* open_sky() {
    ...
    std::shared_ptr<GlassMaterial> glass_material = std::make_shared<GlassMaterial>(1.5f); // 1.5 = Index of Refraction (IOR) for glass
    ...
    green_sphere->model = std::make_unique<Sphere>(0.5, vec3(0.0f), glass_material); 
    root_node->childnodes.push_back(std::move(green_sphere));
    root_node->childtransforms.push_back(translate(vec3(0.0f, 0.0f, 1.0f)));
