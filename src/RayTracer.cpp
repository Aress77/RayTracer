...
Ray RayTracer::ray_thru_pixel(int i, int j) {
     ...
     */
    float screen_x = (i + x) / static_cast<float>(camera.width);
    float screen_y = (j + y) / static_cast<float>(camera.height);

    // Map NDC to screen space coordinates in the range [-1, 1]
    // Note: screen_y is flipped to account for the typical image coordinate origin.
    screen_x = 2.0f * screen_x - 1.0f;
    screen_y = -2.0f * screen_y + 1.0f;

    // Calculate scaling from the field of view.
    // 'scale' is based on the vertical fovy.
    float scale = tan(camera.fovy * 0.5f);

    // Compute the aspect ratio.
    float aspect_ratio = static_cast<float>(camera.width) / camera.height;

    // Calculate alpha and beta
    float alpha = screen_x * aspect_ratio * scale;  // TODO: Implement this
    float beta = screen_y * scale;   // TODO: Implement this
